...
export default function PitcherDashboard() {
  ...
  const calcCMJHandler = () => {
    ...
    if (!bw||isNaN(sf)||isNaN(tf)||isNaN(lf)||tf<=sf||lf<=tf) {setCmjErr('Check your inputs — make sure all frame numbers are entered correctly.');return}
  ...
  return (
    <div style={{fontFamily:'system-ui,-apple-system,sans-serif',background:C.bg,minHeight:'100vh',color:C.text,maxWidth:480,margin:'0 auto'}}>
      ...
      {/* Tab Bar */}
      <div style={{display:'flex',background:C.bg2,borderBottom:`1px solid ${C.border}`,padding:'0 8px'}}>
        {[
          {id:'program',icon:'📋',label:'Program'},
          {id:'log',icon:'📝',label:'Log'},
          {id:'cmj',icon:'🦘',label:'CMJ'},
          {id:'messages',icon:'💬',label:`Messages${unread>0?` (${unread})`:''}`,},
          {id:'notes',icon:'📌',label:'Notes'},
        ].map(t=>(
          <button key={t.id} onClick={()=>setTab(t.id)} style={{flex:1,background:'transparent',border:'none',borderBottom:`2px solid ${tab===t.id?C.gold:'transparent'}`,padding:'10px 4px',cursor:'pointer',color:tab===t.id?C.gold:C.textMuted,fontSize:10,fontWeight:tab===t.id?700:400,textTransform:'uppercase',letterSpacing:'0.3px'}}>
      ...
      <div style={{padding:'16px'}}>
        ...
        {tab==='program'&&(
          <div>
            ...
            {program&&DAYS.map(day=>{
              ...
              if (!hasContent) return <div key={day} style={{...card,opacity:0.5}}><div style={{fontSize:12,fontWeight:700,color:C.textMuted,textTransform:'uppercase',letterSpacing:'0.5px'}}>{day}</div><div style={{fontSize:13,color:C.textDim,marginTop:4}}>Rest</div></div>
              return (
                <div key={day} style={card}>
                  <div style={{fontSize:13,fontWeight:700,color:C.gold,textTransform:'uppercase',letterSpacing:'0.5px',marginBottom:10}}>{day}</div>
                  {CATS.map(cat=>{
                    const content = program.days?.[day]?.[cat]
                    if (!content) return null
                    return (
                      <div key={cat} style={{marginBottom:10}}>
                        <div style={{fontSize:10,color:C.textMuted,fontWeight:700,textTransform:'uppercase',letterSpacing:'0.5px',marginBottom:4}}>{cat}</div>
        ...
        {tab==='log'&&(
          <div>
            ...
            <div style={card}>
              ...
              <textarea style={{...inp,minHeight:80,resize:'vertical'}} placeholder="How did the session feel?" value={logForm.notes} onChange={e=>setLogForm(f=>({...f,notes:e.target.value}))}/>
        ...
        {tab==='cmj'&&(
          <div>
            ...
            <div style={{fontSize:12,color:C.textMuted,marginBottom:16}}>Film at 240 FPS · Open in Photos · Edit · Scrub to find times</div>
            <div style={card}>
              <label style={lbl}>Date</label>
              ...
              <input type="number" style={inp} placeholder="Time × FPS e.g. 3168" value={cmjForm.startFrame} onChange={e=>setCmjForm(f=>({...f,startFrame:e.target.value}))}/>
            ...
            {cmjResult&&(
              <div style={{...card,border:`1px solid rgba(163,113,247,0.4)`,background:'rgba(163,113,247,0.06)',textAlign:'center'}}>
                <div style={{fontSize:11,color:'#a371f7',fontWeight:700,textTransform:'uppercase',letterSpacing:'1px',marginBottom:8}}>Estimated Velocity Capacity</div>
                ...
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:12}}>
                  {[
                    {l:'Jump Height',v:`${cmjResult.jumpHeightIn.toFixed(1)} in`},
                    {l:'RSI-Mod',v:`${cmjResult.rsiMod.toFixed(2)} m/s`},
                    {l:'Peak Power/kg',v:`${cmjResult.peakPowerPerKg.toFixed(1)} W/kg`},
                    {l:'Explosive Index',v:cmjResult.explosiveIndex.toFixed(1)},
                  ].map(m=>(
          ...
          <div>
            ...
            <div style={card}>
              <div style={{display:'flex',flexDirection:'column',gap:10,minHeight:200,marginBottom:16}}>
                ...
                {messages.map((m:any)=>(
                  <div key={m.id} style={{display:'flex',flexDirection:'column',alignItems:m.sender_role==='pitcher'?'flex-end':'flex-start'}}>
          ...
          <div>
            ...
            {notes.map((n:any)=>(
              <div key={n.id} style={card}>
                <div style={{fontSize:10,color:C.textMuted,marginBottom:8,textTransform:'uppercase',letterSpacing:'0.5px',fontWeight:600}}>Coach Salzman · {new Date(n.created_at).toLocaleDateString()}</div>

...
export default function CoachDashboard(){
  ...
  const [editingProgramItem,setEditingProgramItem]=useState<{key:string,idx:number}|null>(null)
  ...
  const openPicker=(day:string,cat:string,editItem?:{idx:number,ex:any})=>{
    ...
    if (editItem){
      ...
      setVideoInput(exerciseVideos[editItem.ex.id]||'')
      setEditingProgramItem({key:`${day}___${cat}`,idx:editItem.idx})
    } else {
      setAddForm(null);setVideoInput('');setEditingProgramItem(null)
    }
    setShowPicker(true)
  }
  ...
  const confirmAddExercise=async()=>{
    ...
    if (editingProgramItem){
      const list=[...(structuredDays[editingProgramItem.key]||[])]
      list[editingProgramItem.idx]=newItem
      newStructured={...structuredDays,[editingProgramItem.key]:list}
    } else {
      const key=`${pickerCell.day}___${pickerCell.cat}`
      const current=structuredDays[key]||[]
      newStructured={...structuredDays,[key]:[...current,newItem]}
    }
    setStructuredDays(newStructured)
    await saveProgram(newStructured,cellNotes)
    setAddForm(null);setShowPicker(false);setEditingProgramItem(null)
  }
          ...
          <div style={{background:C.bg2,border:`1px solid ${C.border}`,borderRadius:12,width:'100%',maxWidth:580,maxHeight:'82vh',display:'flex',flexDirection:'column' as const}}>
            ...
            {addForm&&(
              <div style={{padding:18,overflowY:'auto' as const,flex:1}}>
                <button onClick={()=>{setAddForm(null);setEditingProgramItem(null)}} style={{background:'transparent',border:'none',color:C.textMuted,cursor:'pointer',fontSize:12,marginBottom:14}}>Back to library</button>

...
// Savant zones 1-9 in zone, 10-14 shadow/chase
// Layout grid: 5 cols x 5 rows (corners empty)
// Row 0: _, 11(high), 12(high-mid), 13(high), _
// Row 1: 14(left), 1, 2, 3, 14(right)
// Row 2: 14(left), 4, 5, 6, 14(right)
// Row 3: 14(left), 7, 8, 9, 14(right)
// Row 4: _, 10(low), 10(low-mid), 10(low), _
const ZONE_GRID = [
  [null,11,12,13,null],
  [14,1,2,3,14],
  [14,4,5,6,14],
...
type ChaseRow = {pitch_type:string,zone:number,chase_pct:number,whiff_pct:number,hard_hit_pct:number,xwoba:number,count:number}
type PitchRank = {pitch_type:string,value:number,count:number}

function heatColor(val:number, min:number, max:number, metric:string): string {
  if (max === min) return '#1c2333'
  const t = (val - min) / (max - min)
  // Blue (cold/low) to Red (hot/high)
  // For xwoba: low is good (blue), high is bad (red)
  // For whiff/chase: high is good but we still use red=high for consistency
...
function textForBg(val:number, min:number, max:number): string {
  if (max === min) return C.textMuted
  const t = (val - min) / (max - min)
  return t > 0.45 ? '#ffffff' : C.text
}
...
cat > app/components/PitchingIQ.tsx << 'EOF'
'use client'
import { useEffect, useState, useCallback } from 'react'
import { createClient } from '@/lib/supabase/client'
...
export default function PitchingIQ() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // Chase rows - out of zone locations ranked by chase rate
      ...
      // Pitch rankings by selected metric
      ...
      // For xwoba, lower is better so sort ascending; others sort descending
      ...
      if (chaseList.length > 0 && ranks.length > 0) {
        ...
        const danger = ranks.find(r => r.value > 35 && metric === 'hard_hit')
        ...
        if (danger) msg += ` Avoid ${danger.pitch_type} — ${danger.value.toFixed(0)}% hard-hit rate.`
        else if (ranks[0]) msg += ` Best ${METRIC_LABELS[metric].toLowerCase()}: ${ranks[0].pitch_type} (${metric === 'xwoba' ? ranks[0].value.toFixed(3) : ranks[0].value.toFixed(0) + '%'}).`
        setInsight(msg)
      }
  ...
  const pill = (label:string, active:boolean, onClick:()=>void) => (
    <button key={label} onClick={onClick} style={{
      padding:'4px 10px',borderRadius:20,border:`1px solid ${active?C.gold:C.border}`,
      fontSize:11,cursor:'pointer',color:active?C.bg:C.textMuted,
      background:active?C.gold:'transparent',fontWeight:active?700:400,whiteSpace:'nowrap' as const,
    }}>{label}</button>
  )
  ...
  return (
    <div style={{color:C.text,fontSize:13}}>
      ...
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12,flexWrap:'wrap' as const,gap:8}}>
        ...
        {lastUpdated && (
          <div style={{fontSize:11,color:C.textMuted}}>
            Updated {lastUpdated} · yesterday's games
          </div>
        )}
      </div>
        ...
        <div style={{display:'flex',flexWrap:'wrap' as const,gap:12}}>
          <div>
            ...
            <div style={{display:'flex',gap:4}}>
              {pill('vs RHH', bats==='R', ()=>setBats('R'))}
              {pill('vs LHH', bats==='L', ()=>setBats('L'))}
            </div>
          </div>
          <div>
            ...
            <div style={{display:'flex',gap:4,flexWrap:'wrap' as const}}>
              {ARM_BUCKETS.map(b=>pill(ARM_LABELS[b], armBucket===b, ()=>setArmBucket(b)))}
            </div>
          </div>
          <div>
            <div style={{fontSize:10,color:C.textMuted,textTransform:'uppercase' as const,letterSpacing:'0.5px',marginBottom:4}}>Count</div>
            <div style={{display:'flex',gap:4,flexWrap:'wrap' as const}}>
              {COUNT_BUCKETS.map(b=>pill(b==='all'?'All':b, countBucket===b, ()=>setCountBucket(b)))}
            </div>
          </div>
      ...
      {/* Insight callout */}
      {insight && (
        <div style={{background:'rgba(232,184,75,0.08)',border:`1px solid ${C.goldDim}`,borderRadius:8,padding:'10px 14px',marginBottom:12,fontSize:12,color:C.gold,lineHeight:1.5}}>
          <span style={{fontWeight:700}}>Insight: </span>{insight}
        </div>
      )}

      {loading ? (
        <div style={{textAlign:'center',padding:40,color:C.textMuted}}>Loading pitch data...</div>
      ) : (
        <>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(280px,1fr))',gap:12,marginBottom:12}}>
            ...
            <div style={{background:C.bg2,border:`1px solid ${C.border}`,borderRadius:8,padding:14}}>
              <div style={{fontSize:11,fontWeight:700,color:C.textMuted,textTransform:'uppercase' as const,letterSpacing:'0.5px',marginBottom:10}}>
                Zone heatmap — {METRIC_LABELS[metric]}
              </div>
                ...
                {ZONE_GRID.map((row, ri) => (
                  <div key={ri} style={{display:'flex',gap:3}}>
                    {row.map((zone, ci) => {
                      ...
                      const bg = d && val > 0 ? heatColor(val, minVal, maxVal, metric) : C.bg3
                      const fg = d && val > 0 ? textForBg(val, minVal, maxVal) : C.textDim
                      const displayVal = metric === 'xwoba' ? val.toFixed(3) : val.toFixed(0) + '%'
                      return (
                        <div key={`${ri}-${ci}`} style={{
                          width:52,height:52,borderRadius:4,background:bg,
                          border:`${isInZone?'1.5px':'1px'} solid ${isInZone?'rgba(255,255,255,0.2)':C.border}`,
                          display:'flex',flexDirection:'column' as const,alignItems:'center',justifyContent:'center',
                          opacity: isInZone ? 1 : 0.75,
              ...
              <div style={{textAlign:'center',fontSize:10,color:C.textDim,marginTop:8}}>
                catcher's view · plate_x = left/right · plate_z = height · zones 1–9 in-zone · 10–14 shadow/chase
              </div>
              {/* Legend */}
              ...
              {/* Pitch type ranking */}
              <div style={{fontSize:11,fontWeight:700,color:C.textMuted,textTransform:'uppercase' as const,letterSpacing:'0.5px',marginBottom:8}}>
                Pitch ranking — {METRIC_LABELS[metric]}
              </div>
              {pitchRanks.slice(0,7).map((r,i) => {
                const maxRank = pitchRanks[0]?.value || 1
                const pct = maxRank > 0 ? (r.value / maxRank) * 100 : 0
                const color = i === 0 ? C.red : i === 1 ? '#e8b84b' : C.blue
            ...
            <div style={{background:C.bg2,border:`1px solid ${C.border}`,borderRadius:8,padding:14}}>
              ...
              {chaseRows.length === 0 ? (
                <div style={{color:C.textMuted,fontSize:12,padding:20,textAlign:'center'}}>Not enough data</div>
              ) : (
                chaseRows.slice(0,8).map((row,i) => {
                  ...
                  return (
                    <div key={row.pitch_type} onClick={()=>setSelectedChase(row)} style={{
                      display:'flex',alignItems:'center',gap:7,padding:'6px 6px',
                      borderBottom:`1px solid ${C.border}`,cursor:'pointer',
                      background:isSel?'rgba(232,184,75,0.06)':'transparent',
                      ...
                      <span style={{fontSize:12,fontWeight:600,color:'#f85149',width:32,textAlign:'right' as const}}>{row.chase_pct.toFixed(0)}%</span>
                    </div>
                  )
                })
              )}

              {/* Selected pitch breakdown */}
              {selectedChase && (
                <>
                  <div style={{height:1,background:C.border,margin:'12px 0'}}/>
                  <div style={{fontSize:11,fontWeight:700,color:C.textMuted,textTransform:'uppercase' as const,letterSpacing:'0.5px',marginBottom:8}}>
                    {selectedChase.pitch_type} · out of zone · breakdown
                  </div>
                  {[
